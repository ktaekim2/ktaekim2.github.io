...
이러한 $() 함수를 통해 생성된 요소를 제이쿼리 객체(jQuery object)라고 하며, 생성된 제이쿼리 객체의 메소드를 사용하여 여러 동작을 설정할 수 있음.
...
- js에서는 window 객체의 onload() method를 이용하여 문서가 모두 로드된 뒤에 코드가 실행되도록 설정.
...
- jquery에서는 Document 객체의 ready() method를 이용하여 같은 결과를 보장.
...
<body>
    <h1>현재페이지와 창이 모두 로드되고 나서</h1>
    <p>결과 보기를 다시 눌러보면서 차이를 살펴보세요.</p>
